...
/// <summary>
/// Where the center gaze ray meets a detected AR plane. Used by placement and by wall calibration.
/// </summary>
public class GazePlaneProbe
{
    private Transform origin;
    private ARPlaneManager planeManager;
    private float minDistanceMeters = 0.15f;
    private float maxDistanceMeters = 10f;

    public void Configure(
        Transform gazeOrigin,
    ...
    public bool TryGetIntersection(
        ...
        bool requireVerticalSurface,
        float maximumVerticalNormalDot)
    {
        hitPosition = Vector3.zero;
        hitRotation = Quaternion.identity;
        hitDistance = 0f;

        if (origin == null || planeManager == null || !planeManager.enabled)
            return false;

        Vector3 gazeDirection = origin.forward.normalized;
        if (gazeDirection.sqrMagnitude < 0.0001f)
            return false;

        Ray gazeRay = new Ray(origin.position, gazeDirection);
    ...
    private bool IsPointInsidePlaneBoundary(ARPlane plane, Vector3 worldPoint)
    {
        ...
        Unity.Collections.NativeArray<Vector2> boundary = plane.boundary;
