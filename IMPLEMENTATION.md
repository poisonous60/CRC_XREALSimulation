...
3. Spatial anchors. In the scene anchors are on: `useSpatialAnchors`, `createSpatialAnchorOnQr`, `parentMarkerToAnchor`, and `loadAnchorsOnStart` are all 1, although the script defaults are off. Each Placement creates an `ARAnchor` through `ARAnchorManager`, waits 0.5 s, then saves it through the XREAL anchor persistence (up to 3 attempts, 2 s apart); the old anchor is erased on rescan. On start, after a 1 s delay and up to 8 s waiting for the anchor subsystem, every saved anchor is loaded, and the code follows the XREAL Anchors sample by waiting up to 30 s for a loaded anchor to report tracking before using it. A loaded anchor whose record also has a room pose is ignored, so with the wall flow in use every sphere comes back through layer 2. The XREAL documentation says to look around before saving so the anchor has enough feature points; the build does not enforce a duration.
...
Three variants for the device day, same scene, settings only, no code change:

| Build | Settings | Notes |
|---|---|---|
| placement-only | leave the receiver enabled, run without a server; turn off "hide markers until server connected" and "hide markers without fresh radiation data" on the marker manager | the Scan button needs the receiver present; a failed connection attempt logs an error but does not block scanning or placing |
| server-only | disable the marker manager component only | the HUD shares its object and stays on; the room frame object is created by the manager and disappears with it. To keep the QR camera closed, turn off "start QR camera after connect" on the receiver, which is on in the scene |
